...
def windows_environment(request: pytest.FixtureRequest):
    msi_path: str = request.config.getoption("--msi-path")
    if not msi_path:
        raise ValueError("MSI path is required for Windows environment")
    environment = LRRWindowsEnvironment(msi_path)
    environment.setup()
    request.session.lrr_windows_environment = environment # Store environment in pytest session for access in hooks
    yield
